...
// —————————————————————————  Motors
int motor_left[] = {  
  MTRLFTPIN1, MTRLFTPIN2};
int motor_right[] = {  
  MTRGHTPIN1, MTRGHTPIN2};
byte lastAction[MSGLEN-1];
// ————————————————————————— Setup
void setup() {
if (DEBUG) 
  Serial.begin(9600);
...
void loop() {
  ...
  Aread[0] = analogRead(0);
  Aread[1] = analogRead(1);
  ...
  if ((Aread[0] <200 && lastState[0] > 0) || (Aread[0] >200 && lastState[0] == 0))
  {
    lastState[0] = 1-lastState[0];
    if (Aread[0] >200)
      counters[0]++;
  }
  if ((Aread[1] <200 && lastState[1] > 0) || (Aread[1] >200 && lastState[1] == 0))
  {
    lastState[1] = 1-lastState[1];
    if (Aread[1] >200)
      counters[1]++;
  }
...
//        {
//          counters[0] = counters[0]-counters[1];
//          counters[1]=0;
//        }
//        else
//        {
//          counters[1] = counters[1]-counters[0];
//          counters[0]=0;
//        }
//      }
      for (int i=0; i<MSGLEN-1;i++) 
        lastAction[i] = data[i+1]; 
  ...
  if (lastAction[0] == 1 || lastAction[0] == 2)
  {
    if (counters[0] > counters[1] && lastAction[1] > 128)
      analogWrite(ENABLEMOTORL, --lastAction[1]);
    if (counters[0] < counters[1] && lastAction[1] < 254)
      analogWrite(ENABLEMOTORL, ++lastAction[1]);
    if (counters[0] >= lastAction[3])
      motorL_stop();
    if (counters[1] >= lastAction[3])
      motorR_stop();
    if (counters[0] >= lastAction[3] && counters[1] >= lastAction[3])
      lastAction[0] = 0;
  }
  if (lastAction[0] == 3 || lastAction[0] == 4)
  {
    if (counters[0] + counters[1]>= lastAction[3])
    {
      motorL_stop();
      motorR_stop();
      lastAction[0] = 0;
